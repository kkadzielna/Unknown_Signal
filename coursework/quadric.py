...
def poly_x(xs):
    ones = np.ones(xs.shape)
    xs = np.column_stack((ones, xs, xs**2, xs**3, xs**4))
    return xs
...
def polynomial(X, Y):
    xs = poly_x(X)
    a, b, c, d, e = fit_maximum_likelihood_estimate(xs, Y)
    ys = a + b * X + c * X**2 + d * X**3 + e * X**4
    return ys
...
def kfold(k, data_xs, data_ys, func_type):
    ...
    for i in range(k):
        ...
        if func_type == polynomial:
            ...
            a, b, c, d, e = fit_maximum_likelihood_estimate(train_xs, train_ys)
            yh_test = a + b * test_xs + c * test_xs**2 + d * test_xs**3 + e * test_xs**4
